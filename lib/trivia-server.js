...
game.on('connection', (socket) => {
  ...
  socket.on('guess', payload => {
    ...
    memory.guesses % memory.participants === 0 ?  endTurn(memory.score, payload.all) : game.to(team).emit('message', 'awaiting the other team....');
  });
});
...
function getQuestions() {
  ...
  const url = `https://opentdb.com/api.php?amount=1&category=${category}&difficulty=medium&type=multiple`;
  superagent.get(url)
    .then( result => {
...
function endTurn(score, questions) {
  console.log(score);
